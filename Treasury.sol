...
contract PUMPTreasury is Ownable {
    ...
    address public KIN;
    uint public KINQueue; // Delays change to KIN address
    ...
    /**
        @notice allow approved address to deposit an asset for PUMP
        @param _amount uint
    ...
    function deposit( uint _amount, address _token, uint _profit ) external returns ( uint send_ ) {
        ...
        // mint PUMP needed and store amount of rewards for distribution
        send_ = value.sub( _profit );
        IERC20Mintable( PUMP ).mint( msg.sender, send_ );
    ...
    /**
        @notice allow approved address to burn PUMP for reserves
        @param _amount uint
        @param _token address
     */
    function withdraw( uint _amount, address _token ) external {
        ...
        uint value = valueOf( _token, _amount );
        IPUMPERC20( PUMP ).burnFrom( msg.sender, value );
    ...
    function incurDebt( uint _amount, address _token ) external {
        ...
        uint maximumDebt = IERC20( KIN ).balanceOf( msg.sender ); // Can only borrow against KIN held
    ...
    /**
        @notice allow approved address to repay borrowed reserves with PUMP
        @param _amount uint
     */
    function repayDebtWithPUMP( uint _amount ) external {
        require( isDebtor[ msg.sender ], "Not approved" );

        IPUMPERC20( PUMP ).burnFrom( msg.sender, _amount );

        debtorBalance[ msg.sender ] = debtorBalance[ msg.sender ].sub( _amount );
        totalDebt = totalDebt.sub( _amount );

        emit RepayDebt( msg.sender, PUMP, _amount, _amount );
    }
    ...
    function mintRewards( address _recipient, uint _amount ) external {
        ...
        IERC20Mintable( PUMP ).mint( _recipient, _amount );
    ...
    function excessReserves() public view returns ( uint ) {
        return totalReserves.sub( IERC20( PUMP ).totalSupply().sub( totalDebt ) );
    }
    ...
    /**
        @notice returns PUMP valuation of asset
        @param _token address
        @param _amount uint
        @return value_ uint
     */
    function valueOf( address _token, uint _amount ) public view returns ( uint value_ ) {
        if ( isReserveToken[ _token ] ) {
            // convert amount to match PUMP decimals
            value_ = _amount.mul( 10 ** IERC20( PUMP ).decimals() ).div( 10 ** IERC20( _token ).decimals() );
    ...
    function queue( MANAGING _managing, address _address ) external onlyManager() returns ( bool ) {
        ...
        } else if ( _managing == MANAGING.KIN ) { // 9
            KINQueue = uint32(block.timestamp).add32( secondsNeededForQueue );
        } else return false;
